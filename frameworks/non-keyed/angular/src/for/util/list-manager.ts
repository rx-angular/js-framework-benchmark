...
export function observeTemplateByNotificationKind<U>(templateObserver: any) {
    return (o$: { pipe: (arg0: MonoTypeOperatorFunction<any>) => any; }) => o$.pipe(tap((n: any) => {
        if (n.kind === 'rxError') {
...
export function createListManager<T, C extends any>(config: {
    ...
    const viewContainerRef = config.viewContainerRef;
    const templateRef = config.templateRef;
    const createViewContext = config.createViewContext;
    const defaultStrategyName = config.defaultStrategyName;
    const strategies = config.strategies;
    const trackBy = config.trackBy;
    const cdRef = config.cdRef;
    const renderParent = config.renderParent;
    const eRef = config.eRef;

    const distinctBy = config && config.distinctBy || ((a: T, b: T) => a === b);
